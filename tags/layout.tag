<layout>
    ...
    <div class= "layout" ondrop="{loadZip}">
    ...
    <script>
        ...
        resume() {
            this.loadContent({
                values: riot.mixin('utils').load('values'),
                files: riot.mixin('utils').load('files'),
                objects: riot.mixin('utils').load('objects')
            })
        }
        ...
        loadZip(e) {
            ...
            try {
                ...
                if ( f ) {
                    console.log( 'reading zip: ' + f.name )
                    if ( f.type && f.type.match( 'application/x-zip-compressed' ) != null ) {
                        JSZip.loadAsync(f).then(function(zip) {
                            zip.file("layout.ambuilder").async("string").then(function(builder) {
                                builder = JSON.parse(builder)
                                builder.files = []
                                var readNextFile = function() {
                                    var next = builder.fileList[0]
                                    if ( next ) {
                                        console.log('   reading file: ' + next.name )
                                        zip.file('resources/' + next.name).async("base64").then(function(base64) {
                                            next.data = 'data:' + next.mime + ';base64,' + base64
                                            builder.files.push(next)
                                            builder.fileList.shift()
                                            if ( builder.fileList.length > 0 ) {
                                                readNextFile()
                                            } else {
                                                delete builder.fileList
                                                layout.loadContent(builder)
                                            }
                                        })
                                    } else {
                                        console.warn('zip read incomplete?')
                                    }
                                }
                                if ( builder.fileList.length > 0 ) readNextFile()
                            })
                        })
                    }
                }
            } catch(e) { console.error('Error reading layout zip file: ' + e) }
        }

        //save to a zip file
        saveZip() {
            //generate a list of files for JSON source
            var fileList = []
            for ( var i = 0; i < this.config.files.length; i++ )
                fileList.push({
                    id: this.config.files[i].id,
                    type: this.config.files[i].type,
                    label: this.config.files[i].label,
                    name: this.config.files[i].name,
                    mime: this.config.files[i].mime
                })
            
            var zip = new JSZip()
            zip.file('layout.ambuilder', JSON.stringify({
                "values": this.values,
            ...
            this.config.files.forEach(function(file) {
                resources.file( file.name, file.data.replace('data:;base64,', '').replace('data:image/png;base64,', '').replace('data:image/jpeg;base64,', '').replace('data:image/gif;base64,', '').replace('data:image/bmp;base64,', ''), { base64: true } )
            })
